...
module.exports = function (debug, socket, data) {
    ...
    if (payload.result && payload.id) {
        ...
        if (payload.error) {
            ...
            if (typeof this.awaitingResults[payload.id] === 'function') {
                this.awaitingResults[payload.id].apply(this, [payload.error]);
            }
            return;
        }
        ...
        if (typeof this.awaitingResults[payload.id] === 'function') {
            this.awaitingResults[payload.id].apply(this, [null, payload.result]);
        }
        return;
    }
    ...
    if (typeof this.methods[payload.method] === 'function') {
        debug('← (%s) %s: %o', payload.id, payload.method, payload.params);
        try {
            this.methods[payload.method].apply(request);
        } catch (e) {
